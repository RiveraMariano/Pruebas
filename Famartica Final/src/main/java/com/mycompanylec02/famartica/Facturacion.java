...
public class Facturacion extends Carrito {
    ...
    public void facturarCarrito(){
        if(carro.getMascarillas() == 0 && carro.getSalbutamol() == 0 && carro.getParacetamol() == 0) {
            ...
            } else if(carro.getSalbutamol() > 3){
                JOptionPane.showMessageDialog(null, "Por comprar más de 3 bombas de salbutamol obtiene un 4% de descuento en sus compras");
                setDescuento((float) 0.04);
            } else if(carro.getParacetamol()> 7){
                JOptionPane.showMessageDialog(null, "Por comprar más de 7 pastillas de paracetamol obtiene un 12% de descuento en sus compras");
                setDescuento((float) 0.12);
            }
            setTotal(carro.getSubtotal() - (carro.getSubtotal() * getDescuento()));
            setTotal(getTotal() + (getTotal() * getImpuestos()));
            JOptionPane.showMessageDialog(null, "********FACTURA DE COMPRAS********" + "\nTiene: " + carro.getMascarillas() + " Mascarilla/s" + "\nTiene: " + carro.getSalbutamol() + " Antibiótico/s" + "\nTiene: " + carro.getParacetamol() + " Vacuna/s para Influencia" + "\nSubtotal: ₡" + carro.getSubtotal() + "\nImpuestos: " + getImpuestos() + "%" + "\nDescuento: " + getDescuento() + "%" +  "\n********TOTAL DE COMPRAS********" +"\nTotal: ₡" + getTotal());
        }
